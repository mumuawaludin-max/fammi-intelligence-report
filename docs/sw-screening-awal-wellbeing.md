...
- **Jumlah pegawai per unit dibaca dari daftar induk** (sheet `Wellbeing Assessment ...`, fungsi `bacaDaftarInduk`), bukan dari kolom "Jumlah pegawai" sheet 07. Kolom sheet 07 itu sama dengan jumlah baris Personal Form, sehingga setiap unit tampil 100% mengisi (laporan 2026-09-17: SMA Athirah Baruga dan SMP Athirah Kajaolalido). Daftar induk berbentuk blok `Screening N | <unit>` dengan baris bernomor. Blok Departemen Kurikulum memuat empat unit, dan pergantiannya ditandai baris berjabatan `Kepala Seksi <unit>`: Kurikulum, QGDP TK dan SD, QGDP SMP dan SMA, dan Pendidikan Inklusi. Nama blok dicocokkan ke unit kalau semua katanya ada di nama unit ("ASRAMA BARUGA" ke "Asrama Athirah Baruga"). Unit yang tidak ada di daftar induk kembali memakai sheet 07. Kalau pengisi lebih banyak dari daftar induk (Departemen Kurikulum: 15 pengisi, 14 nama), jumlah pegawai disamakan dengan jumlah pengisi; angka asli daftar induk disimpan di `nPegawaiInduk`. Untuk Athirah yang sudah ter-seed, perbaikan ini dipasang lewat `web/sw-data-lokal/athirah.tambalan-jumlah-pegawai.sql`, yang hanya mengubah `sw_unit.data`. Seed ulang penuh tidak dipakai karena `delete from sw_dataset` ikut menghapus catatan tindak lanjut di `sw_tinjauan` (cascade).
...
1. Jalankan migration `supabase/migrations/20260917100000_sw_screening_awal_wellbeing.sql`, lalu `20260917120000_sw_yayasan_lihat_semua_unit.sql` dan `20260918100000_sw_unit_binaan_pimpinan.sql`. Uji lokalnya ada di `supabase/tests/sw_baseline.sql` dan `supabase/tests/sw_verify.sql`, 23 skenario, semuanya lulus.
2. Tempel `<slug>.seed.sql` di SQL Editor.
3. Aktifkan modul `sw` untuk sekolah itu di Admin Fammi.
4. Buat akun dengan peran `HumanCapital`, `KepalaUnit`, atau `Pegawai`. Isi `profiles.sw_unit_id` untuk kepala unit dan `profiles.sw_individu_id` untuk pegawai. Akun kepala unit dan pegawai bisa diunggah lewat Admin Fammi (Pengguna & Peran, tab Upload CSV / Excel). Satu unit boleh punya beberapa akun kepala unit: template "Direktur dan Wakil Direktur" berisi empat baris (Direktur, Wakil Direktur Wilayah Kajaolalido, Baruga, dan Bone), dan jabatan itu juga dikenali kalau ditulis di kolom Unit. Daftarnya ada di `JABATAN_PIMPINAN` (`web/src/pages/admin/importers/swImporter.js`) karena data screening hanya mencatat "Wakil Direktur Sekolah" tanpa wilayah.
...
| `KepalaUnit` | kepalaUnit | Agregat unitnya sendiri. Semua angka ringkas (pengisi, dinilai atasan, peserta) milik unit; angka lembaga hanya muncul sebagai pembanding yang diberi label (garis tegak, "Pembanding, rata-rata lembaga", baris "Seluruh lembaga (pembanding)"). Tanpa nama. Tanpa tab Pandangan Atasan. Akun pimpinan (Direktur, Wakil Direktur) melihat semua unit binaannya, lihat bawah. |
...
Sejak 2026-09-17 (keputusan pemilik produk) unit dengan pengisi di bawah `asumsi.minPengisiUnit` (bawaan 10) tetap tampil bagi Yayasan, Human Capital, dan kepala unit (unitnya sendiri). Alasannya: baris "unit kecil (digabung)" dan unit yang hilang dari Daftar Peserta, Per Unit, dan Pandangan Atasan memancing pertanyaan. Sakelar "Sembunyikan unit kecil" milik Human Capital ikut dihapus. Ambang masih dipakai untuk tiga hal: pegawai di unit kecil dibandingkan dengan lembaga, bukan dengan unitnya; unit kecil diberi catatan "angkanya mudah berubah" dan tanda "(kecil)" di tabel Per Unit; peringkat "layak dipelajari" dan "paling perlu dukungan" hanya memakai unit di atas ambang. Aturan tampil ditegakkan di dua tempat: di RLS `sw_unit` (migration `20260917120000`), dan di satu fungsi React `unitBolehTampil` yang dipakai semua layar.

**Unit binaan pimpinan** (keputusan pemilik produk 2026-09-18): Direktur menilai para wakil direktur dan kepala departemen, tiap wakil direktur menilai kepala sekolah di wilayahnya. Pembagian ini dibaca dari kolom `dinilai <pimpinan>` di daftar induk (`bacaCakupanPimpinan`, hasilnya `meta.cakupanPimpinan`). Penilai dikenali dari jabatannya: `direktur` = jabatan persis Direktur, `wadir <wilayah>` = jabatan memuat "wakil direktur" dan nama wilayahnya, boleh disingkat ("kajol" untuk Kajaolalido). Tiap orang yang dinilai diterjemahkan jadi satu unit: kalau ia sekantor dengan penilainya (para wakil direktur yang dinilai direktur), unitnya kantor itu; kalau tidak, unit yang pegawainya paling banyak menyebut dia sebagai atasan langsung. Athirah: Direktur 12 unit (kantor direksi dan 11 unit departemen), Wadir Kajaolalido 4 sekolah, Wadir Baruga 5 (TK, SD, SMP, SMA, dan Asrama Baruga), Wadir Bone 4 (SD, SMP, SMA, dan Boarding Bone). Nama orang yang dinilai tidak disimpan karena `meta` terbaca semua peran. Akun pimpinan tetap `KepalaUnit`; unit binaannya disimpan di tabel `sw_unit_binaan` (migration `20260918100000`), diisi `create-user` saat akun diunggah dari Admin Fammi (hanya untuk baris di unit Direktur dan Wakil Direktur, supaya akun kepala SMA Bone milik Wakil Direktur Bone tetap satu sekolah). RLS `sw_unit` memakai `my_sw_unit_ids()`: unit binaan kalau ada, kalau tidak `profiles.sw_unit_id`. Di layar, akun dengan beberapa unit mendapat pemilih unit di Ringkasan dan Suara Pegawai, tabel multi-unit di Per Unit, dan baris per unit di Daftar Peserta. Empat akun pimpinan Athirah yang sudah dibuat diisi lewat `web/sw-data-lokal/athirah.tambalan-unit-binaan.sql`.
...
- Tidak ada unduh CSV. Satu-satunya ekspor berisi nama adalah Excel di tab Daftar Peserta, sub-tampilan "Semua pegawai", khusus Human Capital (keputusan pemilik produk 2026-09-23). Tab itu punya dua sub-tampilan yang dipilih di kartu ringkasan paling atas: "Peserta asesmen lanjutan (200)" (tampilan lama: tujuh saringan, grafik per alasan dan per unit, daftar nama di dialog) dan "Semua pegawai" (seluruh pengisi, peserta maupun bukan, dalam satu tabel yang bergulir di dalam kartunya). Saringannya cuma satu, Unit. Urutan bawaannya Prioritas: peserta asesmen lanjutan (masuk daftar 200) di atas, diurutkan dari skor prioritas final tertinggi, lalu pegawai lainnya dengan aturan yang sama; urutan lain yang bisa dipilih: Unit lalu nama, dan Nama. Baris peserta berlatar ungu muda (`--fm-ungu` 14% di atas putih, `#E9E0FA`) dengan garis kiri ungu, dan kolom Cara masuk diganti "Asesmen lanjutan" berisi label Peserta. Berkas Excel berisi persis baris yang tampil dengan urutan layar: No, Nama, Peserta asesmen lanjutan (Ya/Tidak), Unit, Jabatan, Jenjang, Alasan, Pandangan atasan, Cara masuk, Sumber data. Baris peserta di Excel diberi latar warna yang sama dengan "Ya" tebal; kepala kolom beku dan bersaringan otomatis; sheet Keterangan memuat lembaga, periode, unit, urutan, jumlah peserta, waktu unduh, dan legenda warna. Angka skor prioritas tidak pernah ikut diekspor; prioritas hanya terbaca dari urutan baris. Berkas dibuat di browser dari baris yang sudah lolos RLS dengan `exceljs` (dimuat saat tombol ditekan; SheetJS versi gratis tidak bisa menulis warna sel), kode di `lib/swEkspor.js`, uji di `lib/swEkspor.test.js`. Yayasan dan kepala unit tidak mendapat sub-tampilan ini maupun ekspornya.
