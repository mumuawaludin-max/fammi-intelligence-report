...
1. Jalankan migration `supabase/migrations/20260917100000_sw_screening_awal_wellbeing.sql`, lalu `20260917120000_sw_yayasan_lihat_semua_unit.sql`. Uji lokalnya ada di `supabase/tests/sw_baseline.sql` dan `supabase/tests/sw_verify.sql`, 23 skenario, semuanya lulus.
...
| `KepalaUnit` | kepalaUnit | Agregat unitnya sendiri. Semua angka ringkas (pengisi, dinilai atasan, peserta) milik unit; angka lembaga hanya muncul sebagai pembanding yang diberi label (garis tegak, "Pembanding, rata-rata lembaga", baris "Seluruh lembaga (pembanding)"). Tanpa nama. Tanpa tab Pandangan Atasan. |
