...
10. **Angka pencapaian karakter dibaca dari rekap sekolah, tidak pernah dihitung ulang** (2026-09-14). Sumber tunggal untuk "berapa persen pencapaian karakter sekolah X bulan Y" adalah `karakter_summary` scope `sekolah`, yaitu salinan apa adanya sheet ringkasan sekolah di berkas unggahan. Urutannya tiga tingkat dan cuma ada satu implementasi per sisi: `nilaiGuruSekolah()` di `web/src/pages/karakter/karakterMeta.js` untuk React, `ypt_rekap_sekolah_guru` (di atas fungsi `ypt_pct()`) di Postgres, keduanya wajib berubah bareng. Tingkatnya: (1) `rata_pencapaian_guru`, (2) rata-rata kolom `rata_input_guru_karakterN_*` di baris yang sama, (3) `pencapaian_guru` sebagai pilihan terakhir. **`pencapaian_guru` tanpa awalan `rata_` berarti KELENGKAPAN INPUT, bukan pencapaian karakter**; melompat dari tingkat 1 langsung ke tingkat 3 adalah bug yang sudah dua kali terjadi dan membuat lima TK Telkom tampil 100% padahal 68-98% (diukur pada 78 baris rekap YPT 2026-03..05: 68 di antaranya punya `pencapaian_guru` yang beda lebih dari 5 poin dari pencapaian sebenarnya). Tingkat 3 tetap dipertahankan karena sebagian sekolah di luar YPT memang menaruh angka pencapaiannya di kolom itu. Konsekuensinya untuk butir 9: saringan `skor > 0` dan `sumber = 'guru'` di `ypt_k_sekolah_mat`/`ypt_k_aspek_mat` sekarang cuma berlaku pada jalur CADANGAN (sekolah-bulan yang rekapnya belum masuk) dan pada `jumlah_siswa` yang dipakai sebagai bobot; `ypt_k_indikator_mat` dan `ypt_k_siswa_ekstrem_mat` tetap sepenuhnya dari skor karena rekap tidak punya kolom per indikator maupun per murid. Di sekolah yang sheet detail dan sheet rekapnya tidak sejalan, kartu sekolah dan daftar murid memang bisa bercerita beda; itu QC berkas di hulu, bukan alasan menyamakan paksa kedua sumber.
...
**Pengecualian yang disengaja (4)**: modul Screening Awal Wellbeing (`web/src/pages/sw/`, kode modul `sw`, 2026-09-17) memakai palet desain acuan pemilik produk lewat custom property `--fm-*` di `swTokens.module.css` (class `.scope`): ungu tua `#342060`, emas `#FFAE15`, jingga `#FF5B2E`, hijau `#16A34A`, latar `#F5F5F7`. `--fm-ungu` menunjuk `--purple-600` karena nilainya sama. Kartu tidak memakai garis tepi; pembedanya warna latar dan bayangan tipis (`--fm-bayangan`), atas instruksi pemilik produk. Keterangan di bawah judul bagian dan catatan selalu satu baris. Tata letaknya ruang kendali untuk laptop 1366x768: tiap tab muat satu layar tanpa gulir (hanya dialog yang boleh bergulir), semua progress bar memakai satu tebal `--fm-bar`, huruf paling kecil 13 px, teks di atas ungu selalu putih, setiap legenda punya tooltip, dan grafik beranimasi saat tab dibuka; rinciannya di dokumen modul. Isian ganda (nama, unit, jabatan sama) dibuang di pembaca, yang dipakai isian terakhir; kursi peserta dibagi ulang dengan aturan sheet 06 supaya tetap 200, dan rata-rata unit/lembaga dihitung ulang dari baris bersih (satu-satunya tempat FIR merata-rata, di CLI pembaca, bukan di React). Tidak ada unduh CSV; satu-satunya ekspor berisi nama adalah Excel "Semua pegawai" di tab Daftar Peserta, khusus Human Capital, satu saringan Unit, urutan bawaan prioritas (peserta asesmen lanjutan di atas, lalu skor prioritas tertinggi), baris peserta berlatar ungu muda di layar maupun di Excel plus kolom "Peserta asesmen lanjutan" Ya/Tidak (keputusan pemilik produk 2026-09-23; angka skor prioritas tetap tidak diekspor; berkas ditulis dengan `exceljs` yang dimuat saat tombol ditekan, karena SheetJS gratis tidak bisa menulis warna sel). Peran Pegawai memakai tampilan ponsel sendiri (`SwPegawaiLaporan`). Modul ini juga memakai `@phosphor-icons/react` untuk ikon, dengan grafik dari CSS/SVG sendiri (tanpa pustaka grafik baru). Pembacanya yayasan dan tim Human Capital, jadi bahasanya sehari-hari: kategori Sangat perlu perhatian, Perlu perhatian, Cukup, Baik, Sangat baik (bukan "menopang"). Kata "kesehatan mental", "gangguan", "sehat", "tidak sehat", dan "waspada" dilarang sebagai label (ada uji yang memeriksanya). Modul ini membawa tiga peran baru: `KepalaUnit` (tertaut lewat `profiles.sw_unit_id`), `HumanCapital` (akses penuh termasuk nama), dan `Pegawai` (tertaut lewat `profiles.sw_individu_id`, shell ponsel sendiri). `KepalaUnit` dan `HumanCapital` hanya melihat modul `sw` walau sekolahnya punya modul lain. Log akses profil dan panel asumsi SENGAJA tidak ada (dikeluarkan atas instruksi pemilik produk); jangan dikembalikan tanpa instruksi baru. Unit dengan pengisi di bawah `asumsi.minPengisiUnit` TETAP tampil bagi Yayasan, Human Capital, dan KepalaUnit (unitnya sendiri) sejak 2026-09-17 (keputusan pemilik produk: laporan harus utuh, tanpa baris "unit kecil digabung" dan tanpa sakelar sembunyikan); jangan dikembalikan ke penggabungan. Ambang hanya menahan unit kecil sebagai pembanding laporan Pegawai, memberi catatan "angkanya mudah berubah", dan mengeluarkan unit kecil dari peringkat samping tab Per Unit. Aturan tampil ditegakkan di RLS `sw_unit` (migration `20260917120000`) dan di satu fungsi React `unitBolehTampil`. Akun pimpinan (Direktur, Wakil Direktur) tetap berperan `KepalaUnit` tapi melihat semua unit binaannya (tabel `sw_unit_binaan`, fungsi `my_sw_unit_ids()`, migration `20260918100000`, keputusan pemilik produk 2026-09-18); pembagiannya dibaca dari kolom "dinilai ..." di daftar induk berkas, bukan diketik manual. Skor final dibaca dari berkas olahan lewat `npm run sw:baca`. Hasil baca berisi nama pegawai dan disimpan di `web/sw-data-lokal/`, yang ada di .gitignore dan tidak boleh di-commit. Rincian dan asumsi terbuka ada di `docs/sw-screening-awal-wellbeing.md`.
