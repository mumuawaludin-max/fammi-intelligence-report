...
--
-- ── JEBAKAN: fungsi yang mengembalikan text[] JANGAN dibungkus di dalam ANY() ────────────────
-- my_yayasan_school_ids() dan my_cakupan() sama-sama returns text[]. Menulis
--   sekolah_id = any ((select my_yayasan_school_ids()))
-- GAGAL dengan "operator does not exist: text = text[]": di dalam ANY(), Postgres membaca
-- `(select ...)` sebagai SUBQUERY, bukan sebagai nilai array, lalu mencoba membandingkan text
-- dengan baris bertipe text[]. Menambah cast `::text[]` pun masih ditolak.
--
-- Bentuk yang dipakai di sini tidak punya ambiguitas itu sama sekali:
--   sekolah_id in (select unnest(my_yayasan_school_ids()))
-- unnest memecah array jadi baris-baris text, jadi ini murni IN(subquery) atas text. Subqueryn-
-- ya tidak berkorelasi dengan baris luar, sehingga perencana tetap mengeksekusinya sekali lalu
-- memakai ulang hasilnya, persis efek yang dicari. Semantiknya identik dengan `= any(array)`,
-- termasuk untuk array kosong dan nilai NULL.
--
-- ── Yang TIDAK berubah ───────────────────────────────────────────────────────────────────────
...
--
-- Lalu ulangi EXPLAIN yang sama; Buffers dan Execution Time harus turun drastis.
...
using (
  ...
  and (
    ...
    or ((select my_peran()) = 'WaliKelas' and kelas_id in (select unnest(my_cakupan())))
...
as permissive for select to authenticated
using (sekolah_id in (select unnest(public.my_yayasan_school_ids())));
...
using (
  ...
  or sekolah_id in (select unnest(my_yayasan_school_ids()))
