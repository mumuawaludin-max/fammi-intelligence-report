...
import { CwShareDialog } from "./CwShareDialog";
import { TIPE_BUDAYA_INFO, implikasiBudaya } from "./cwMeta";
import tokens from "./cwBudayaTokens.module.css";
import styles from "./CwBudayaPerbandinganDumbbell.module.css";
 ...
 * CwDimensiPerbandingan.jsx, prinsip yang sama berlaku di sini. */
...
function buatPesanBagikan(selected, selectedGap, selectedInfo, priorityActions) {
  ...
  const baris = [
    `Halo rekan-rekan, dari hasil asesmen Fammi Corporate Culture & Wellbeing periode ini, ada satu fokus yang ingin saya bagikan ke kita semua: *${selected.tipe}* (${selectedInfo?.ringkas || "-"}).`,
    "",
    `Skor kita saat ini di angka ${formatScore(selected.saat_ini)}%, sementara harapan karyawan ada di ${formatScore(selected.harapan)}%` +
...
/**
 * CwBudayaPerbandinganDumbbell -- bagian "01-B" khusus Budaya Kerja: dumbbell chart saat ini vs
 * harapan + panel prioritas eksekutif. Gap dibaca dari tabel_gap (angka resmi dari hulu), bukan
 * dihitung ulang dari harapan - saat_ini.
 *
 * Salinan ScBudayaPerbandinganDumbbell.jsx modul School Culture, istilah korporat.
 */
export function CwBudayaPerbandinganDumbbell({ sectionIndex, chartData, tabelGap, selected, onSelect, onPrioritize }) {
  ...
  return (
    <section className={`${tokens.scope} ${styles.section}`} id="cw-budaya-perbandingan">
      <CwLaporanReveal className={styles.heading}>
        ...
        <p>Bandingkan posisi keempat tipe budaya pada skala yang sama untuk menemukan prioritas intervensi.</p>
      </CwLaporanReveal>

      <div className={styles.layout}>
        <CwLaporanReveal className={styles.chart} amount={0.1}>
          <div className={styles.chartHeader}>
            <div>
              <h3>Perbandingan 4 tipe budaya</h3>
              ...
              return (
                ...
                >
                  <span className={styles.rowLabel}>
                    <CwIconBadge icon={dimension.tipe} size="sm" />
                    <span>
        ...
        <CwLaporanReveal className={styles.priorityPanel} delay={0.08} amount={0.1}>
          <div className={styles.priorityTitle}>
            <CwIconBadge icon="award" size="sm" tone="gold" />
            <span>
          ...
          {selected && (
            ...
            >
              <CwIconBadge icon={selected.tipe} size="lg" />
              <span>
                <small>Tipe budaya terpilih</small>
            ...
            {priorityActions ? (
              <ul className={styles.focusList}>
                {priorityActions.map((action) => (
                  <li key={action}>
                    <CwIconBadge icon="target" size="sm" tone="plain" />
                    <span>{action}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className={styles.gapNote}>Belum ada rekomendasi tindak lanjut spesifik untuk tipe budaya ini pada periode ini.</p>
            )}
          </div>
