...
/**
 * Jalur baca modul Corporate Culture & Wellbeing.
 *
 * PENTING: modul ini membaca TABEL YANG SAMA dengan School Culture (sc_personal, sc_lembaga,
 * sc_hasil, sc_komitmen) dan baris briefing/tindak_lanjut dengan modul = 'sc'. Itu bukan
 * kelalaian penamaan, tapi keputusan pemilik produk 2026-09-09: instrumen, format berkas, dan
 * importernya memang satu (sudah diuji dengan berkas olahan PT Glamindo), jadi menggandakan
 * tabel cw_* cuma akan menggandakan migrasi, Edge Function, dan layar persetujuan tanpa dapat
 * apa-apa. Yang membedakan kedua modul cuma entitlement school_modules dan tampilannya.
 * Kalau nanti instrumen korporat benar-benar berbeda, barulah pisahkan tabelnya.
 *
 * Berkas ini salinan pages/sc/useScData.js dengan tiga perbedaan yang disengaja:
 *   1. Kalimat yang dibaca pengguna memakai istilah korporat (karyawan, perusahaan), bukan
 *      istilah sekolah.
 *   2. Field hasil dipetakan ke nama yang dipakai komponen CW: cerita_karyawan (bukan
 *      cerita_pegawai), meta.nama_perusahaan (bukan meta.nama_lembaga), meta.jabatan (bukan
 *      meta.peran_kerja), dan jawaban_survey.gambaran_perusahaan (bukan gambaran_lembaga).
 *      Lihat sesuaikanLaporanIndividu di bawah.
 *   3. targetRoleForPeran cuma mengenal Manajemen, satu-satunya level pimpinan di korporat.
 *
 * Sama seperti SC: narasi dirakit lewat template kalimat dari angka yang SUDAH final, bukan
 * menghitung skor baru (CLAUDE.md butir 3), dan hanya baris berstatus 'disetujui' yang dibaca.
 */
 ...
 * pemanggilnya di useCwAgregat (tidak pernah memanggil fungsi ini untuk domain organisasi). */
...
/** Di korporat cuma ada satu level pimpinan, yaitu Manajemen (padanan Yayasan di konteks
 * sekolah). AdminFammi ikut memakai baris target_role yang sama supaya bisa meninjau apa yang
 * dilihat klien. */
function targetRoleForPeran(peran) {
  if (peran === "AdminFammi") return "manajemen";
  return peran === "Manajemen" ? "manajemen" : "kepala_sekolah";
}

/**
 * Samakan nama kunci word cloud "cerita" dengan yang dibaca CwBudayaCeritaKaryawan.jsx. Isi dan
 * jumlah mention-nya tidak disentuh, cuma nama kuncinya.
 */
function sesuaikanCeritaKaryawan(cerita) {
  if (!cerita) return null;
  return {
    gambaran_perusahaan: cerita.gambaran_lembaga || cerita.gambaran_perusahaan || [],
    saat_ini: cerita.saat_ini || [],
    ingin_diubah: cerita.ingin_diubah || [],
  };
}

/**
 * Samakan bentuk sc_hasil.detail dengan yang dibaca CwLaporanIndividuPage.jsx. Isi laporannya
 * TIDAK diubah sama sekali, cuma tiga nama field yang di skema SC memakai istilah sekolah:
 * nama_lembaga, peran_kerja, dan gambaran_lembaga. Nama lama dibiarkan ikut terbawa supaya
 * konsumen lain (mis. layar persetujuan admin) tidak ikut rusak.
 */
function sesuaikanLaporanIndividu(detail) {
  if (!detail) return null;
  return {
    ...detail,
    meta: {
      ...detail.meta,
      nama_perusahaan: detail.meta?.nama_lembaga || detail.meta?.nama_perusahaan || null,
      jabatan: detail.meta?.peran_kerja || detail.meta?.jabatan || null,
    },
    jawaban_survey: detail.jawaban_survey
      ? {
        ...detail.jawaban_survey,
        gambaran_perusahaan:
          detail.jawaban_survey.gambaran_lembaga || detail.jawaban_survey.gambaran_perusahaan || undefined,
      }
      : detail.jawaban_survey,
  };
}
...
/**
 * Laporan agregat Corporate Culture & Wellbeing untuk pimpinan (Manajemen/KepalaSekolah/WakilKepalaSekolah/
 ...
 */
export function useCwAgregat(session, periodeId) {
    ...
    return {
      ...
      header: {
        hook: briefingAtPeriode?.teks
          || `Budaya kerja perusahaan Anda paling condong ke ${dominan?.tipe || "—"} periode ini.`,
        sub_hook: `Ringkasan dari ${sekolah.jumlah_responden || 0} karyawan pada periode ${periodeLabel(periode)}.`,
      },
      bagian_budaya: {
        narasi: gapTop
          ? `Budaya yang paling terasa saat ini adalah ${dominan?.tipe} (${dominan?.mean_gambaran}%). Selisih terbesar antara kondisi sekarang dan harapan karyawan ada pada ${gapTop.tipe} (${gapTop.gap > 0 ? "+" : ""}${gapTop.gap} poin).`
          : "",
      ...
      bagian_kesejahteraan: {
        narasi: kesTertinggi && kesTerendah
          ? `Indeks kesejahteraan gabungan karyawan berada di sekitar ${indeksKesejahteraan}%. ${kesTertinggi.label} jadi subdimensi terkuat (${kesTertinggi.nilai}%), sementara ${kesTerendah.label} paling perlu diperhatikan (${kesTerendah.nilai}%).`
          : "",
      ...
      perbandingan_antarunit: {
        narasi: unitRows.length > 0
          ...
          : "Belum ada data agregat per unit kerja untuk periode ini, baru tersedia ringkasan seluruh perusahaan.",
      ...
      footer: {
        disclaimer: "Laporan ini adalah hasil pengolahan jawaban asesmen seluruh karyawan yang mengisi pada periode berjalan dan bersifat rahasia. Gunakan sebagai bahan pengambilan keputusan organisasi, bukan alat evaluasi individu karyawan tertentu.",
      },
      tema_esai: briefingAtPeriode?.tema_esai || [],
      // Komponen CW menamainya cerita_karyawan; kolom briefing-nya tetap cerita_pegawai karena
      // dipakai bersama School Culture. Kunci word cloud di dalamnya (gambaran_lembaga,
      // saat_ini, ingin_diubah) ikut diseragamkan ke nama yang dipakai CwBudayaCeritaKaryawan.
      cerita_karyawan: sesuaikanCeritaKaryawan(briefingAtPeriode?.cerita_pegawai),
 ...
 * pemetaan ulang di sini -- beda dari useCwAgregat yang memang perlu merakit ulang dari
 * sc_lembaga/tindak_lanjut/briefing.
 */
export function useCwIndividu(session) {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!scRespondenId) {
        setState({ loading: false, error: "Akun ini belum ditautkan ke responden manapun (profiles.sc_responden_id kosong).", data: null });
        return;
      }
      setState((s) => ({ ...s, loading: true, error: null }));
      ...
      setState({ loading: false, error: null, data: data?.detail ? { ...sesuaikanLaporanIndividu(data.detail), approved_at: data.approved_at } : null });
    }
...
export function useCwKomitmen(scRespondenId, periodeId) {
 ...
 * LaporanIndividuSC (lihat useCwIndividu), jadi array ini bisa langsung dipakai sebagai
 ...
 */
export function useCwRespondenList(session, periodeId) {
  ...
  const respondenList = useMemo(() => {
    ...
    return state.rows.filter((r) => r.periode_id === periode).map((r) => ({ ...sesuaikanLaporanIndividu(r.detail), approved_at: r.approved_at }));
